...
sequenceDiagram
    ...
    Note over MatchSvc: 게임메이트 조회, 중복 확인,포인트 부족 확인, 자가 매칭 방지 등유효성 검증 수행
    ...
    Note over CoinSvc: coinAmount: -price (음수)paymentAmount: 0paymentMethod: "MATCH_PENDING"
    ...
    Note over MatchSvc: orderStatus: "PENDING"
...
```
## 9. 매칭 요청 시 코인 차감 (내부 메서드)

| 항목 | 흐름 요약 | 핵심 비즈니스 로직 |
|:---|:---|:---|
| **목표** | 매칭 요청 시 요청자의 코인을 차감하고 거래 내역 기록 | - |
| **호출 시점** | `MatchService.requestMatch()`에서 매칭 요청이 유효성 검증을 통과한 후 호출됩니다. | - |
| **포인트 차감** | `CoinService.useCoinForMatch()`는 요청자의 **point를 매칭 가격만큼 차감**하고 `UserRepository`를 통해 **UPDATE**를 요청합니다. | **포인트 차감** (음수 처리) |
| **거래 기록** | `Service`는 `Coin` 엔티티를 생성하여 **사용 내역을 DB에 INSERT**합니다. | **거래 이력 저장** (coinAmount: 음수, paymentMethod: "MATCH_PENDING") |
