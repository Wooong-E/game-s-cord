...
sequenceDiagram
    ...
    Service->>MatchRepo: findPendingMatch(requesterId, orderedUsersId, ordersGameId)
    MatchRepo->>DB: SELECT * FROM matches WHERE order_status=PENDING
...
| **중복 요청 확인** | `Service`는 `MatchRepository`를 통해 동일한 게임메이트에게 대기 중인 매칭 요청이 있는지 확인합니다. | **중복 매칭 요청 방지** |
| **코인 잔액 확인** | `Service`는 요청자의 보유 코인이 게임메이트 가격보다 충분한지 확인합니다. | **코인 부족 시 요청 거부** |
